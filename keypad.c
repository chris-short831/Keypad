...
Date: 09/29/2022
Overview: This code defines the functions used in main.c. The operation of each
...
//This function initializes the GPIO register to drive the LEDs
void LEDpinInit(void)
{
	//Configure GPIOC pins
		//Connect GPIOC to clock
		RCC->AHB2ENR |= (RCC_AHB2ENR_GPIOCEN);
		GPIOC->MODER &= ~(GPIO_MODER_MODE0 | GPIO_MODER_MODE1 |GPIO_MODER_MODE2 | GPIO_MODER_MODE3); //set MODER Mask = 0
		GPIOC->MODER |= (GPIO_MODER_MODE0_0 | GPIO_MODER_MODE1_0 | GPIO_MODER_MODE2_0 | GPIO_MODER_MODE3_0); //set MODER[0] == 1
		//This sets these bits with MODE[15:0][01] which sets the pins to be in general purpose output mode
		GPIOC->OTYPER &= ~(GPIO_OTYPER_OT0 | GPIO_OTYPER_OT1 | GPIO_OTYPER_OT2 | GPIO_OTYPER_OT3); //sets the output type to Push-Pull
		GPIOC->PUPDR &= ~(GPIO_PUPDR_PUPD0 | GPIO_PUPDR_PUPD1 | GPIO_PUPDR_PUPD2 | GPIO_PUPDR_PUPD3); //set PUPD == 0
		//Sets OSPEED to 3? which is "Very high Speed"
...
/* This function sends the value of the key variable to the LEDs to
be displayed */
void LEDsend(uint32_t key)
{
	/*If a key is pressed, clear the previous LEDbus data and set it with the
	new key variable value */
	if( (columnIDR->IDR & 1) | (columnIDR->IDR & 2) | (columnIDR->IDR & 4))
	{
	LEDbus->BRR = (GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_3);;
	LEDbus->ODR |= key;
	}
}
